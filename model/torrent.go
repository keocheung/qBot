...
type Torrent struct {
	...
	State          string  `json:"state"`            // Torrent state
...
const (
	TorrentStateStop   TorrentState = "stop"
	TorrentStateStart  TorrentState = "start"
	TorrentStateDelete TorrentState = "delete"
)

type TorrentAction struct {
	State    TorrentState `json:"state" yaml:"state"`
...
type Options struct {
	...
	Sort    string `json:"sort"`    // Sort torrents by given key. They can be sorted using any field of the response's JSON array as the sort key.
